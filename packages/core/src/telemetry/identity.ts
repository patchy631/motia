...
export function generateProjectId(workspacePath?: string): string {
  try {
    ...
    if (fs.existsSync(packageJsonPath)) {
      // If package.json exists, use it for project identification
      const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8'))
      projectInfo = `${packageJson.name || 'unknown'}|${packageJson.version || '0.0.0'}`
    } else {
 ...
 */
export function getTelemetryIdentityAttributes(
